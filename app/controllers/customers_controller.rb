...
before_action :signed_in_user, only: [:index, :edit, :update, :destroy]
	...
	def show
		...
		@tasks = Task.where("customer_id = ? AND done = ?", @customer.id,false).order(sort_column + " " + sort_direction)
		#@tasks = Task.where("customer_id = ?", @customer.id).order(sort_column + " " + sort_direction)
		#@tasks = Customer.joins(:tasks).where("customer_id = ?", params[:id]).order(sort_column + " " + sort_direction)
	end
